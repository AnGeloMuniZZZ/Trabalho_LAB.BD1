...
-- Calcula o 2� digito verificador --
		...
		IF (UPPER(@opc) = 'I') BEGIN
			...
			IF (@validaCPF = 1 AND @validaExemplar = 1) BEGIN
				BEGIN TRY
					...
					SET @saida = 'Loca��o inserida com sucesso'
				END TRY
				BEGIN CATCH
					SET @erro = ERROR_MESSAGE()
					RAISERROR(@erro, 16, 1)
...
---------------------------------------------------------------------------- TESTES ----------------------------------------------------------------------------- 
--Fun��o: calcular_primeiro_digito_cpf
...
--Funcao: gerar_ra (ACHO Q NAO PREISA ENVIAR O CPF, VALIDA��O DEVE SER FEITA EM OUTRO LUGAR)
...
-- teste 1 (N�o existe emails identicos, retorna 0)
...
-- teste 2 (cria um e-mail para uma pessoa com nome hom�nimo na tabela, deve retornar "alexandre.oliveira2")
...
-- teste 7 (Tentar Inserir dados invalidos) T� JOGANDO 3 RAISERROS
...
-- teste 8 (Atualizar um Exemplar com dados invalidos)  TA JOGANDO 3 RAIERROS isso � muito erro
